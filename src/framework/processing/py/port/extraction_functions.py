...
def extract_blocked_accounts(blocked_accounts_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in blocked_accounts_dict["relationships_blocked_users"]] # get list with timestamps in epoch format
...
def extract_close_friends(close_friends_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in close_friends_dict["relationships_close_friends"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_following(following_dict):
    ...
    if following_dict == None:
        ...
        following = [f for f in following_dict["relationships_following"]] # get count of following
        count = len(following)
...
def extract_follow_requests_youve_received(follow_requests_youve_received_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in follow_requests_youve_received_dict["relationships_follow_requests_received"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_hide_story_from(hide_story_from_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in hide_story_from_dict["relationships_hide_stories_from"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_pending_follow_requests(pending_follow_requests_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in pending_follow_requests_dict['relationships_follow_requests_sent']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_recently_unfollowed_accounts(recently_unfollowed_accounts_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in recently_unfollowed_accounts_dict["relationships_unfollowed_users"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_removed_suggestions(removed_suggestions_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in removed_suggestions_dict["relationships_dismissed_suggested_users"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_restricted_accounts(restricted_accounts_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in restricted_accounts_dict["relationships_restricted_users"]] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_personal_information(personal_information_dict):
    ...
    for k in ['Phone Confirmed', 'Telefonnummer best\u00c3\u00a4tigt']: # keys are language specific
...
def extract_post_comments_1(post_comments_1_dict):
    ...
    if isinstance(post_comments_1_dict, dict):
        dates = [epoch_to_date(post_comments_1_dict["string_map_data"]["Time"]["timestamp"])]
    else:
        dates = [epoch_to_date(t["string_map_data"]["Time"]["timestamp"]) for t in post_comments_1_dict] # get list with timestamps in epoch format
...
def extract_reels_comments(reels_comments_dict):
    ...
    dates = [epoch_to_date(t["string_map_data"]["Time"]["timestamp"]) for t in reels_comments_dict['comments_reels_comments']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_countdowns(countdowns_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in countdowns_dict['story_activities_countdowns']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_emoji_sliders(emoji_sliders_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in emoji_sliders_dict['story_activities_emoji_sliders']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_polls(polls_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in polls_dict['story_activities_polls']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_questions(questions_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in questions_dict['story_activities_questions']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_quizzes(quizzes_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in quizzes_dict['story_activities_quizzes']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
...
def extract_story_likes(story_likes_dict):
    ...
    dates = [epoch_to_date(t["string_list_data"][0]["timestamp"]) for t in story_likes_dict['story_activities_story_likes']] # get list with timestamps in epoch format
    dates_df = pd.DataFrame(dates, columns=['date']) # convert to df
